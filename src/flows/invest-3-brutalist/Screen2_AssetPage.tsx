/**
 * Screen2_AssetPage — Neo-brutalist asset detail page.
 * Hard edges, 1px borders, no glow/gradients, black CTA pills.
...
// ── Shortcut button (brutalist: hard border circle) ──
...
// ── Detail row (brutalist: 1px border-bottom) ──
...
export default function Screen2_AssetPage({ onNext, onBack, onElementTap }: FlowScreenProps) {
  ...
  return (
    <div className="relative flex flex-col h-full overflow-hidden" style={{ background: BG }}>
      {/* Radial glow — returns transparent in brutalist theme */}
      <div
      ...
      <div className="relative flex-1 overflow-y-auto" style={{ paddingTop: `calc(${SAFE_TOP} + 8px)` }}>
        ...
        >
          ...
          >
            <svg width="24" height="24" fill="none" viewBox="0 0 24 24">
              <path d="M15 19l-7-7 7-7" stroke={TEXT_PRIMARY} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </motion.button>
          ...
          >
            <svg width="24" height="24" fill={isFav ? '#FBBF24' : 'none'} viewBox="0 0 24 24">
              <path
                ...
                stroke={isFav ? '#FBBF24' : TEXT_MUTED}
                strokeWidth="1.5"
                strokeLinecap="round"
        ...
        <motion.div className="flex flex-col items-center gap-2 px-4 mb-6" {...fadeUp(0.05)}>
          ...
          {/* Category badge — brutalist: hard border pill */}
          <span
        ...
        {/* Time range pills (keep rounded-full — pills stay round) */}
          ...
          {TIME_RANGES.map((range, i) => (
            <button
              ...
              style={{
                ...
                color: i === activeRange ? '#FFFFFF' : TEXT_SECONDARY,
                fontSize: 12,
                fontWeight: 600,
            ...
            >
              {i === activeRange && (
                <motion.div
                  ...
                  style={{ background: TEXT_PRIMARY, zIndex: -1 }}
        ...
        {invested && position ? (
          <>
            ...
            <motion.div className="flex items-center justify-center gap-8 px-4 mb-8" {...fadeUp(0.25)}>
              <ShortcutBtn
                icon={
                  <svg width="20" height="20" fill="none" viewBox="0 0 24 24">
                    <path d="M12 5v14M5 12h14" stroke={TEXT_PRIMARY} strokeWidth="2" strokeLinecap="round" />
                  </svg>
                }
                label="Comprar"
                onPress={() => {
              ...
              <ShortcutBtn
                icon={
                  <svg width="20" height="20" fill="none" viewBox="0 0 24 24">
                    <path d="M5 12h14" stroke={TEXT_PRIMARY} strokeWidth="2" strokeLinecap="round" />
                  </svg>
                }
                label="Vender"
                onPress={() => {
              ...
              <ShortcutBtn
                icon={
                  <svg width="20" height="20" fill="none" viewBox="0 0 24 24">
                    <circle cx="12" cy="6" r="1.5" fill={TEXT_PRIMARY} />
                    <circle cx="12" cy="12" r="1.5" fill={TEXT_PRIMARY} />
                    <circle cx="12" cy="18" r="1.5" fill={TEXT_PRIMARY} />
                  </svg>
                }
                label="Mais"
                onPress={() => {
            ...
            {/* Detalhes section — brutalist: rounded-none cards */}
            ...
            {/* Historico section — brutalist: rounded-none */}
          ...
          <>
            ...
            {/* Info card — brutalist: rounded-none */}
      ...
      {/* ── Fixed bottom bar (not invested) — brutalist: black CTA pill ── */}
      <AnimatePresence>
        {!invested && (
          <motion.div
