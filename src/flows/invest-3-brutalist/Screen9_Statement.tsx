/**
 * Statement (Extrato) — neo-brutalist transaction history with filter dropdowns.
 * Three custom filter pills with hard borders, colored transaction rows, PDF export CTA.
  ...
  return (
    <div className="relative flex-1 min-w-0">
      <motion.button
        whileTap={{ scale: 0.96 }}
        onClick={onToggle}
        className="flex items-center gap-1 w-full rounded-none px-2.5 py-2 border-none cursor-pointer"
        style={{
          background: BG_CARD,
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="absolute left-0 right-0 mt-1 rounded-none overflow-hidden z-30"
            style={{
              background: '#FFFFFF',
              border: `1px solid ${BORDER}`,
              transformOrigin: 'top',
          ...
          >
            {options.map(option => (
              <motion.button
                ...
                style={{
                  background: value === option ? BG_CARD : 'transparent',
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <motion.div variants={listContainer} initial="hidden" animate="visible" className="flex flex-col">
            {filtered.map((tx) => {
              ...
              return (
                ...
                >
                  {/* Color dot — square for brutalist */}
                  <div
        ...
        <motion.div {...fadeUp(0.3)} className="px-5 mt-4 mb-6">
          <motion.button
            ...
            className="flex items-center justify-center gap-2 w-full py-3.5 rounded-none border-none cursor-pointer"
            style={{
              background: BG_CARD,
              border: `1px solid ${BORDER}`,
            }}
