/**
 * Dashboard parts — dark-themed sub-components for the visual investments dashboard.
...
export function HoldingRow({ icon, name, ticker, value, quantity, color, sparkline, onPress }: HoldingRowProps) {
  ...
  return (
    <motion.button
      onClick={onPress}
      whileTap={{ scale: 0.97 }}
      className="flex items-center gap-3 w-full px-4 py-3 text-left cursor-pointer border-none"
      style={{
    ...
    >
      ...
      {/* Sparkline */}
      <Sparkline data={sparkline} color={color} width={60} height={24} />
    ...
    >
      ...
      <div className="flex flex-col gap-3 mt-3">
        {results.map((r, i) => (
          ...
          >
            <img src={r.icon} alt={r.ticker} className="rounded-full" style={{ width: 24, height: 24 }} />
            <span className="flex-1" style={{ color: TEXT_PRIMARY, fontSize: 13, fontWeight: 500 }}>
              {r.name}
            </span>
            <Sparkline data={r.sparkline} color={r.color} width={40} height={16} />
