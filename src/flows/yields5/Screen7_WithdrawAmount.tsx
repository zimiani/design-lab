...
export default function Screen7_WithdrawAmount({ onNext, onBack }: FlowScreenProps) {
      ...
      <Stack gap="default">
        ...
        <Stack direction="row" gap="sm">
          {WITHDRAW_QUICK_PICKS.map((pick) => (
            <Button key={pick.label} variant="primary" inverse size="sm" onPress={() => setValue(rawDigitsFromAmount(MOCK_BALANCE * pick.pct))}>{pick.label}</Button>
          ))}
