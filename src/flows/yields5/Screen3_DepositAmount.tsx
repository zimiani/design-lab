...
export default function Screen3_DepositAmount({ onNext, onBack, onElementTap, onStateChange }: FlowScreenProps) {
      ...
      <Stack gap="default">
        <Stack gap="none">
          ...
          {isPix && (
            <>
              <Divider />
              <ListItem title="Você paga" subtitle={isValid ? `R$ ${brlEquivalent.toFixed(2).replace('.', ',')}` : 'Insira um valor'} inverted trailing={null} />
            </>
          )}
          <ListItem
          ...
          <Stack gap="none">
            <DataList
              data={[
                ...
                ...(isPix ? [{ label: 'Câmbio', value: `US$ 1 ⇄ R$ ${MOCK_RATE.toFixed(4).replace('.', ',')}` }] : []),
      ...
      <BottomSheet open={sheetOpen} onClose={() => setSheetOpen(false)} title="Fonte de recursos">
        <Stack gap="none">
          {FUNDING_SOURCES.map((source) => (
            <ListItem key={source.id} title={source.title} subtitle={source.subtitle} left={<Avatar src={source.icon} size="md" />} onPress={() => { setSelectedSource(source.id); setSheetOpen(false) }} />
          ))}
        </Stack>
      </BottomSheet>
    </BaseLayout>
