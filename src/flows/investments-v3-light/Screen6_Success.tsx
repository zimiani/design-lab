/**
 * Success — light theme celebration with confetti and summary.
 */
...
function useParticles(count: number, color: string) {
  return Array.from({ length: count }, (_, i) => ({
    ...
    color: i % 3 === 0 ? color : i % 3 === 1 ? '#10B981' : '#E5E7EB',
    ...
    <div className="relative flex flex-col overflow-hidden" style={{ background: '#FFFFFF', minHeight: '100vh' }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8">
        <motion.div
          ...
          style={{ background: `${asset.color}12`, border: `2px solid ${asset.color}25` }}
        ...
        <motion.div
          className="text-[14px] text-gray-400 text-center mb-8"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        {showContent && (
          <motion.div
            className="w-full rounded-2xl px-5 py-2 bg-gray-50 border border-gray-100"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          >
      ...
      <div className="px-5 pb-[max(var(--safe-area-bottom),24px)]">
        <motion.button
          ...
          className="w-full py-4 rounded-2xl font-bold text-[16px] text-gray-700 bg-gray-100 border border-gray-200"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
