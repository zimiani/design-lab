/**
 * Custom visual components — light theme variant.
 */
...
export function FeaturedCard({ asset, onPress, index }: {
  ...
  return (
    <motion.button
      ...
      style={{
        background: `linear-gradient(135deg, ${asset.color}12 0%, ${asset.color}06 100%)`,
    ...
    >
      <div
        className="absolute -top-12 -right-12 w-24 h-24 rounded-full opacity-15 pointer-events-none"
      ...
      <div className="relative flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-[13px] font-semibold text-gray-900">{asset.name}</div>
            <div className="text-[11px] text-gray-400">{asset.ticker}</div>
          </div>
        </div>
        ...
        <div className="flex items-end justify-between">
          ...
          {!isFixed && (
            <div className={`text-[12px] font-medium ${isPositive ? 'text-emerald-600' : 'text-red-500'}`}>
...
export function AssetListRow({ asset, onPress, index }: {
  ...
  return (
    <motion.button
      onClick={onPress}
      className="flex items-center gap-3 w-full px-1 py-3 text-left border-b border-gray-100 last:border-0"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[14px] font-semibold text-gray-900 truncate">{asset.name}</div>
      ...
      <div className="text-right flex-shrink-0 w-[90px]">
        ...
        {!isFixed && (
          <div className={`text-[11px] font-medium ${isPositive ? 'text-emerald-600' : 'text-red-500'}`}>
            {formatPct(asset.change24h!)}
          </div>
        )}
        {isFixed && (
          <div className="text-[11px] text-emerald-600">Renda Fixa</div>
        )}
      </div>
    </motion.button>
  )
